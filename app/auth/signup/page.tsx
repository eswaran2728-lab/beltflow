...
import { UserRole, ROLE_HOME } from '@/lib/auth';

const roles: { value: UserRole; label: string; desc: string }[] = [
  { value: 'admin',   label: 'Admin / Daerah Admin', desc: 'Full system access, manage all data' },
  { value: 'coach',   label: 'Coach / Master',        desc: 'Manage assigned classes and students' },
  { value: 'parent',  label: 'Parent / Guardian',     desc: 'View your child\'s progress and payments' },
  { value: 'student', label: 'Student',               desc: 'View your own profile and achievements' },
];
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await signup({
      ...
      assignedClassIds: role === 'coach' && form.assignedClass ? [form.assignedClass] : undefined,
    });
    setLoading(false);
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Join BeltFlow — Free MVP</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {roles.map(r => (
                  <label key={r.value}
                    className={`cursor-pointer border-2 rounded-xl p-3 transition-all ${role === r.value ? 'border-[#0f172a] bg-gray-50' : 'border-gray-100 hover:border-gray-200'}`}>
            ...
            <div>
              ...
              <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                placeholder="e.g. Ahmad bin Razak" required
            ...
            {role === 'parent' && (
              <div className="bg-green-50 border border-green-100 rounded-xl p-4">
                ...
                <p className="text-xs text-green-600 mt-1">Admin will link your account to the student record.</p>
              </div>
            )}
            {role === 'coach' && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                <label className="block text-sm font-medium text-blue-900 mb-1">Assigned Class (optional)</label>
                <input type="text" value={form.assignedClass} onChange={e => setForm({...form, assignedClass: e.target.value})}
                  placeholder="e.g. Junior A, Senior B"
                ...
                <p className="text-xs text-blue-600 mt-1">Admin will confirm your class assignment.</p>
              </div>
            )}
        ...
        <p className="text-center text-xs text-gray-400 mt-3">BeltFlow MVP · Free for Daerah Sepang Testing</p>
      </div>
    </div>
  );
}
